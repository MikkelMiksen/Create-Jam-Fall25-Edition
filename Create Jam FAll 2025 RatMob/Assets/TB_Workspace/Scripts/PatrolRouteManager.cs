...
public class PatrolRouteManager : MonoBehaviour
{
        ...
        IEnumerator Start()
        {
                ...
                {
                        patrollingEntity.GetComponentInChildren<PatrolUnit>().isPatrolManagerReady = true;
                }
        }
        ...
        public void GetYourPatrolRoute(EntityTypes entityType)
        {
                ...
                {
                        PatrolUnit.Instance.route.AddRange(routs[entityType]);
                        Debug.Log(entityType + " had rout assigned to it's list");
                }

                if (PatrolUnit.Instance.route.Count == 0)
                {
